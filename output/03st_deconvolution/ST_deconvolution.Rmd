...
NBH_TS_2_unfiltered_spe.rds <- readRDS("NBH_TS_2_unfiltered_spe.rds")

# Define the pattern to remove
#Obteined from here: https://doi.org/10.1038/s41598-020-62801-6
pattern_to_remove <- "Bc1|^mt-|^Hba1-|^Hba2-|^Hbb-|^Hbbp1-|^Hbd-|^Hbe1-|^Hbg1-|^Hbg2-|^Hmm-|^Hbq1-|^Hbz-|^Hbzp1- |^Hba-"

# Find the indices of genes that match the pattern
genes_to_remove <- grep(pattern_to_remove, NBH_TS_2_unfiltered_spe.rds@rowRanges@elementMetadata$Symbol)

# Remove the genes from the rowRanges
NBH_TS_2_unfiltered_spe.rds@rowRanges <- NBH_TS_2_unfiltered_spe.rds@rowRanges[-genes_to_remove, ]

# Update the counts accordingly
NBH_TS_2_unfiltered_spe.rds@assays@data$counts <- NBH_TS_2_unfiltered_spe.rds@assays@data$counts[-genes_to_remove, ]
...
#__________________________________________DECONVOLUTION OF INDIVIDUAL SPOTS________________________________________
## this is the genes x barcode sparse count matrix
```{r pressure, echo=FALSE}
cd <- NBH_TS_2_unfiltered_spe.rds@assays@data@listData$counts
pos <- SpatialExperiment::spatialCoords(NBH_TS_2_unfiltered_spe.rds)
...
for (i in seq_along(ps)) {
  ggsave(plot_names[i], plot = ps[[i]], device = "svg", width = 2, height = 2, dpi = 300, path = "/Users/davidecaredio/spatial_prions/output/03st_deconvolution/Deconvolved_Topics/RML6_30_1/Spatial_Expression") # Specify your path
}
...
```{r pressure, echo=FALSE}
geneSymbols <- NBH_TS_2_unfiltered_spe.rds@rowRanges@elementMetadata$Symbol
names(geneSymbols) <- names(NBH_TS_2_unfiltered_spe.rds@rowRanges)
geneSymbols[1:5]
...
ps <- lapply(colnames(deconProp), function(celltype) {
  ...
  plt <- ggplot2::ggplot(data = dat) +
    ...
    ggplot2::theme(axis.text.x = ggplot2::element_text(size=6, color = "black", angle = 90),
...
for (i in seq_along(ps)) {
  ggsave(plot_names[i], plot = ps[[i]], device = "svg", width = 1, height = 2, dpi = 300, path = "/Users/davidecaredio/spatial_prions/output/03st_deconvolution/Deconvolved_Topics/RML6_30_1/DEGs") # Specify your path
}


library(openxlsx)

# Create an empty data frame to combine all data
combined_data <- data.frame()

# Iterate over each data frame in the list ps
for (i in 1:length(ps)) {
  
  # Extract the data frame
  df <- ps[[i]][["data"]]
  
  # Get the first gene (this will be the "Topic")
  topic_name <- df$genes[1]
  
  # Add the "Topic" column to the data frame, filled with the first gene name
  df$Topic <- topic_name
  
  # Combine the current data frame with the overall data
  combined_data <- rbind(combined_data, df)
}
# Write the modified data frames to an Excel file, with each data frame in a separate sheet
write_xlsx(combined_data, "RML6_30wpi_1rep.xlsx")
...
#____________________________________COMPARE TO TRANSCRIPTIONAL CLUSTERING_________________________________________
```{r pressure, echo=FALSE}
